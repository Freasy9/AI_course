...
- **✳️ COSTAR提示词**：与上者同款条形图，演示提示完整度（教学分）与创作工坊 COSTAR 字段一致；可一键「生成文本」走与 **COSTAR 创作工坊** 相同的 `generateCostarStageOutput` 管线，并「同步到创作工坊」将普通提示词 + 六维写入工坊（`sessionStorage` 一次性同步），自动切到创作工坊页
